...
class TestTask5(unittest.TestCase):
    ...
    def test_results_have_required_keys(self):
        ...
        search = self._import_task5()
        try:
            results = search("tuition fee payment", top_k=3)
            if not results:
    ...
    def test_results_sorted_descending(self):
        ...
        search = self._import_task5()
        try:
            results = search("university tuition policy", top_k=5)
            if len(results) < 2:
...
class TestTask7(unittest.TestCase):
    ...
    def test_rerank_has_score(self):
        ...
        rerank_fn = self._import_task7()
        candidates = [
            {"content": "Tuition fee payment policy", "score": 0.7, "metadata": {}},
            {"content": "Library study room booking guide", "score": 0.5, "metadata": {}},
        ]
        try:
            results = rerank_fn("tuition fee", candidates, top_k=2)
            if results:
...
class TestTask8(unittest.TestCase):
    ...
    def test_returns_list_with_source_marker(self):
        ...
        search = self._import_task8()
        try:
            results = search("tuition fee", top_k=2)
            self.assertIsInstance(results, list)
            if results:
...
class TestTask9(unittest.TestCase):
    ...
    def test_retrieve_returns_list(self):
        ...
        try:
            results = retrieve_fn("tuition fee payment", top_k=3)
            self.assertIsInstance(results, list)
        except NotImplementedError:
    ...
    def test_results_have_required_keys(self):
        ...
        retrieve_fn = self._import_task9()
        try:
            results = retrieve_fn("university tuition policy", top_k=3)
            if not results:
                self.skipTest("Không có kết quả")
            for r in results:
                self.assertIn("content", r)
